<script setup lang="ts">
  ...
  const {data: posts} = await useAsyncData<Post[]>('fetchPosts', () => {
    return directus.request<Post[]>(readItems('Post', {
      fields: ['*', {
        medias: ['*']
      }]
    }))
...
<template>
  <div class="row q-mt-lg justify-center">
    <div v-for="(post, postId) in posts" :key="postId" class="col-12" style="max-width: 600px;">
      <a v-if="post.sourceUrl" :href="post.sourceUrl" target="_blank">
        <div class="row">
          ...
          <div class="col-12">
            <div class="row items-center justify-between">
              ...
              <div class="col-auto text-grey">
                {{ new Date(post.date_created as string).toLocaleDateString('de', {dateStyle: 'long'}) }}
              </div>
            </div>
          </div>
          <div v-if="post.medias.length" class="col-12"><img :src="(post.medias[0] as PostMedia).imageUrl as string" style="max-width: calc(100% + (2*8px)); margin-left: -8px; margin-right: -8px;" /></div>
          ...
          <div class="col-12"><p class="text-subtitle1 q-mb-none">{{ post.title }}</p></div>
          <div v-if="post.lead" class="col-12"><p class="text-caption q-mt-xs q-mb-none" >{{ post.lead }}</p></div>
        </div>
      </a>
    </div>
  </div>
